...
(defun run-journaled-test (thunk &key (enabled t))
  ...
  (let ((cl-weave::*journal-enabled* enabled)
        (test (cl-weave::make-test-case :name "journal subject" :function thunk)))
    (cl-weave::run-test-case (cl-weave::root-suite) test)))

(defun event-journal (event)
  (cl-weave::test-event-journal event))

(defmethod cl-weave:journal-frame-line-for-kind ((kind (eql :test-suite-custom-frame-kind)) frame)
  (format nil "custom frame #~D" (cl-weave:journal-frame-index frame)))

(describe
  "execution journal"
  (it
    "does not record a timeline unless journaling is enabled"
    (let ((event
          (run-journaled-test
            (lambda ()
              (expect 1 :to-be 1))
  ...
  (it
    "is inactive outside of a journaled attempt"
    (expect (cl-weave::journaling-active-p) :to-be nil))
  (it
    "records passing matcher assertions when enabled"
    (let* ((event
          (run-journaled-test
            (lambda ()
              (expect 1 :to-be 1))))
  ...
  (it
    "records failing assertions before the failure unwinds"
    (let* ((event
          (run-journaled-test
            (lambda ()
              (expect 1 :to-be 2))))
  ...
  (it
    "records a chronological, index-ordered timeline"
    (let* ((event
          (run-journaled-test
            (lambda ()
              (expect 1 :to-be 1)
      ...
      (expect (mapcar #'cl-weave:journal-frame-index frames) :to-equal '(0 1 2))
      (expect (every #'cl-weave:journal-frame-pass frames) :to-be t)))
  (it
    "records smart predicate assertions with their operands"
    (let* ((event
          (run-journaled-test
            (lambda ()
              (expect (= 2 2)))))
      ...
      (expect (cl-weave:journal-frame-pass frame) :to-be t)))
  (it
    "records failing smart predicate assertions"
    (let* ((event
          (run-journaled-test
            (lambda ()
              (expect (= 1 2)))))
...
(describe
  "execution journal REPL helpers"
  (it
    "collects frames from a body via with-execution-journal"
    (let ((frames (cl-weave:with-execution-journal (expect 1 :to-be 1) (expect (= 2 2)))))
  ...
  (it
    "records labelled notes and returns their value"
    (let* ((result nil)
           (frames
  ...
  (it
    "treats journal-note as a transparent no-op when inactive"
    (expect (cl-weave:journal-note :ignored 7) :to-be 7)))

(describe
  "execution journal spec reporter"
  (it
    "renders the timeline and replay seed for failed events"
    (let* ((assertion
  ...
  (it
    "omits the timeline and seed for passing events"
    (let ((output
          (with-output-to-string (stream)
            (cl-weave::report-spec
              (list (make-sample-event :status :pass :path '("journal" "clean")))
              stream))))
      (expect output :not :to-contain "timeline")
      (expect output :not :to-contain "replay seed"))))

(describe
  "execution journal mock frames"
  (it
    "records mock invocations on the timeline"
    (let* ((frames
          (cl-weave:with-execution-journal
      ...
      (expect (cl-weave:journal-frame-actual (first mock-frames)) :to-equal '(1 2))))
  (it
    "does not record mock calls when journaling is inactive"
    (let ((mock (make-mock-function)))
      (funcall mock :x)
      (expect (cl-weave::journaling-active-p) :to-be nil)))
  (it
    "renders mock-call frames in the spec reporter"
    (let* ((frame
          (cl-weave::make-journal-frame :index 0 :kind :mock-call :actual '(1 2) :pass t))
           (output
          (with-output-to-string (stream)
            (cl-weave::report-spec
              (list
                (make-sample-event :status :fail :path '("mocks" "spec") :journal (list frame)))
              stream))))
      (expect output :to-contain "#0 mock-call (1 2)"))))

(describe
  "execution journal hook frames"
  (it
    "records before-each and after-each hooks around the body"
    (let* ((order '())
           (suite
          (cl-weave::make-suite
            :name
      ...
      (expect (cl-weave::test-event-status event) :to-be :pass)
      (expect
        (mapcar #'cl-weave:journal-frame-kind frames)
        :to-equal
        '(:hook :assertion :hook))
  ...
  (it
    "flags a failing after-each hook on its frame"
    (let* ((suite
          (cl-weave::make-suite
            :name
            "root"
          ...
          (find
            ...
            (cl-weave::test-event-journal event)
            :key
            #'cl-weave:journal-frame-kind)))
      (expect (cl-weave:journal-frame-form hook-frame) :to-be :after-each)
      (expect (cl-weave:journal-frame-pass hook-frame) :to-be nil)))
  (it
    "renders hook frames in the spec reporter"
    (let* ((frame
          (cl-weave::make-journal-frame :index 0 :kind :hook :form :before-each :pass t))
           (output
          (with-output-to-string (stream)
            (cl-weave::report-spec
              (list
                (make-sample-event :status :fail :path '("hooks" "spec") :journal (list frame)))
              stream))))
      (expect output :to-contain "#0 hook before-each -> ok"))))

(describe
  "execution journal shrink frames"
  (it
    "records an accepted shrink-step frame for each step down to the minimal case"
    (let* ((generator
      ...
      (expect (every #'cl-weave:journal-frame-pass frames) :to-be t)))
  (it
    "records a rejected frame for a candidate that does not reproduce the failure"
    (let* ((generator
          (cl-weave::make-property-generator
            :name
            :binary
      ...
      (expect
        (mapcar
          (lambda (frame)
            (list (cl-weave:journal-frame-form frame) (cl-weave:journal-frame-pass frame)))
          frames)
        :to-equal
        '(((0) nil) ((1) t)))))
  (it
    "does not record shrink frames when journaling is inactive"
    (let ((generator
          (cl-weave::make-property-generator
            :name
            :ordered
      ...
      (expect (cl-weave::journaling-active-p) :to-be nil)))
  (it
    "renders shrink-step frames in the spec reporter"
    (let* ((frame
          (cl-weave::make-journal-frame
            :index
            0
          ...
          (with-output-to-string (stream)
            (cl-weave::report-spec
              (list
                (make-sample-event :status :fail :path '("shrink" "spec") :journal (list frame)))
              stream))))
...
(describe
  ...
  (it
    "turns a frame into keyed relation facts, omitting matcher when absent"
    (let* ((assertion
          (cl-weave::make-journal-frame
            :index
            0
          ...
          (cl-weave::make-journal-frame
            ...
            t)))
      (expect
        (cl-weave:journal-facts (list assertion))
        :to-equal
        '((:frame 0)
          (:kind 0 :assertion)
          (:form 0 (expect 1 :to-be 2))
          (:actual 0 1)
          (:expected 0 2)
          (:pass 0 nil)
          (:elapsed-internal-time 0 5)
          (:matcher 0 :to-be)))
      (expect
        (cl-weave:journal-facts (list note))
        :to-equal
        '((:frame 1)
          (:kind 1 :note)
          (:form 1 :total)
  ...
  (it
    "queries a timeline for failing assertions with journal-where"
    (let* ((frames
          (cl-weave:with-execution-journal
            (ignore-errors
              (cl-weave:with-soft-assertions
                ...
                (expect (= 4 5)))))))
      (expect
        (cl-weave:journal-where frames (:kind ?index :assertion) (:pass ?index nil))
        :to-contain-equal
        '((?index . 1)))
      (expect
        (cl-weave:journal-where frames (:kind ?index :assertion) (:pass ?index nil))
        ...
        '((?index . 2)))))
  (it
    "accepts an already-expanded fact program directly"
    (let ((program
          (cl-weave:journal-facts
            (cl-weave:with-execution-journal (ignore-errors (expect 1 :to-be 2))))))
      (expect
        (cl-weave:query-journal program '((:kind ?i :assertion)))
        :to-equal
        '(((?i . 0))))))
  (it
    "composes derived rules over the journal to find failing assertions"
    (let* ((frames
          (cl-weave:with-execution-journal
            (ignore-errors
              (cl-weave:with-soft-assertions
          ...
          (append
            ...
            (cl-weave:logic-program
              (:- (:failed-assertion ?index) (:kind ?index :assertion) (:pass ?index nil))))))
      (expect
        (cl-weave:journal-where program (:failed-assertion ?index))
        :to-equal
        '(((?index . 1)) ((?index . 2)))))))
...
(describe
  "journal breakpoints"
  (it
    "matches an integer breakpoint against a frame's index"
  ...
  (it
    "matches a function breakpoint by calling it with the frame"
    (let ((frame (cl-weave::make-journal-frame :index 0 :kind :assertion :pass nil)))
      (expect
        (cl-weave::journal-breakpoint-match-p
          (lambda (f)
            (not (cl-weave:journal-frame-pass f)))
          frame)
        :to-be
        t)))
  (it
    "normalizes nil, integers, and functions; rejects everything else"
  ...
  (it
    "signals journal-breakpoint-hit with the matched frame when a handler intercepts it"
    (let ((seen nil))
  ...
  (it
    "never signals when no frame matches the breakpoint"
    (let ((hit nil))
      (handler-bind ((cl-weave:journal-breakpoint-hit
            (lambda (condition)
  ...
  (it
    "does nothing when *journal-breakpoint* is unbound"
    (let ((hit nil))
      (handler-bind ((cl-weave:journal-breakpoint-hit
            (lambda (condition)
              (declare (ignore condition))
...
(describe
  "journal-diff"
  (it
    "returns nil for timelines that differ only in timing"
    (let ((a
          (list
            (cl-weave::make-journal-frame
              :index
  ...
  (it
    "reports the first mismatching frame"
    (let* ((a
          (list
            (cl-weave::make-journal-frame :index 0 :kind :assertion :actual 1 :pass t)
  ...
  (it
    "reports length divergence when one timeline is shorter"
  ...
  (it
    "finds no divergence between two deterministic runs"
...
(describe
  "explain-journal"
  (it
    "renders each frame kind on its own line as a string"
    (let* ((frames
          (list
            (cl-weave::make-journal-frame :index 0 :kind :hook :form :before-each :pass t)
            (cl-weave::make-journal-frame :index 1 :kind :mock-call :actual '(42) :pass t)
            (cl-weave::make-journal-frame :index 2 :kind :assertion :form :truthy :pass nil)
            (cl-weave::make-journal-frame
              :index
              3
              :kind
  ...
  (it
    "writes to a provided stream and returns no values"
    (let* ((frame
          (cl-weave::make-journal-frame
            :index
            0
           ...
           (output
          (with-output-to-string (stream)
            (setf values (multiple-value-list (cl-weave:explain-journal (list frame) stream))))))
      (expect output :to-contain "#0 assertion")
...
(describe
  ...
  (it
    "confirms no divergence for a nil diff"
    (expect
      (cl-weave:explain-journal-diff nil)
      :to-equal
      "no divergence: both timelines matched"))
  (it
    "renders a mismatch diff with both frames rendered by journal-frame-line"
    (let* ((a
          (cl-weave::make-journal-frame
            :index
            1
          ...
          (cl-weave::make-journal-frame
            ...
            nil))
           (text
          (cl-weave:explain-journal-diff (list :index 1 :a a :b b :reason :mismatch))))
  ...
  (it
    "renders a missing side as a placeholder on a length divergence"
    (let* ((a (cl-weave::make-journal-frame :index 2 :kind :assertion :pass t))
           (text
          (cl-weave:explain-journal-diff (list :index 2 :a a :b nil :reason :length))))
  ...
  (it
    ...
    (let* ((values nil)
           (output
          (with-output-to-string (stream)
            (setf values (multiple-value-list (cl-weave:explain-journal-diff nil stream))))))
      (expect output :to-equal "no divergence: both timelines matched")
      (expect values :to-equal nil))))

(describe
  "journal frame line extensibility"
  (it
    "uses the default renderer for kinds without a specialized method"
    (let ((frame
          (cl-weave::make-journal-frame
            :index
            0
            ...
            t)))
      (expect
        (cl-weave:journal-frame-line frame)
        :to-equal
        "#0 unspecialized-kind :PAYLOAD -> ok")))
  (it
    "dispatches to a user-defined eql-specialized method for a custom kind"
    (let ((frame
          (cl-weave::make-journal-frame
            :index
            5
            :kind
            :test-suite-custom-frame-kind
            :pass
            t)))
      (expect (cl-weave:journal-frame-line frame) :to-equal "custom frame #5")))
  (it
    "records a custom-kind frame directly through the now-public record-journal-frame"
    (let ((frames
          (cl-weave:with-execution-journal
      ...
      (expect
        (cl-weave:journal-frame-line (first frames))
        :to-equal
        "#0 custom-event :HELLO -> ok"))))

(describe
  "execution journal sexp reporter"
  (it
    "serializes journal frames and the replay seed"
    (let* ((frame
          (cl-weave::make-journal-frame
            :index
            0
  ...
  (it
    "emits an empty timeline and null seed by default"
    (let ((output
          (with-output-to-string (stream)
            (cl-weave::report-sexp
              (list (make-sample-event :status :pass :path '("journal" "clean")))
...
(describe
  "execution journal JSON reporter"
  (it
    "emits an empty timeline for events without a journal"
    (let ((output
          (with-output-to-string (stream)
            (cl-weave::report-json
              (list (make-sample-event :status :pass :path '("journal" "empty")))
              stream))))
      (expect output :to-contain "\"timeline\":[]")))
  (it
    "serializes journal frames as timeline entries"
    (let* ((frame
          (cl-weave::make-journal-frame
            :index
            0
